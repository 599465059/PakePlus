...
(function() {
  ...
  class App {
          ...
          <div class="split-section">
            ...
            <div class="config-list" id="configList">
              ${configs.length ? configs.map(c => `
                <div class="control-list-item" data-name="${c.name}">
                  <div class="name">${c.name}${defaultName === c.name ? ' ⭐' : ''}</div>
                  <div class="type">${c.desc || '无描述'}</div>
                  <div style="margin-top:8px;">
                    <button style="padding:4px 8px;background:#22c55e;color:white;border:none;border-radius:4px;font-size:11px;margin-right:4px;" onclick="App.setDefaultConfig('${c.name}')">设为默认</button>
                    <button style="padding:4px 8px;background:#dc3545;color:white;border:none;border-radius:4px;font-size:11px;" onclick="App.deleteConfigFile('${c.name}')">删除</button>
                  </div>
                </div>
      ...
      if (configList) {
        configList.querySelectorAll('.control-list-item').forEach(item => {
          item.addEventListener('click', () => {
            ...
            panel.innerHTML = `
              <div style="margin-bottom:16px;">
                <div style="font-size:16px;font-weight:600;color:#2c3e50;margin-bottom:8px;">${config.name}</div>
                <div style="font-size:13px;color:#5a6a7a;">${config.desc || '无描述'}</div>
              </div>
              <div style="font-size:13px;color:#2c3e50;margin-bottom:12px;">
                <div>📄 页面数：${config.pages}</div>
                <div>📦 控件数：${config.controls}</div>
              </div>
              <button class="btn-primary" style="width:100%;margin-bottom:8px;" onclick="App.applyConfigByName('${config.name}')">应用此配置</button>
              <button class="btn-danger" style="width:100%;" onclick="App.deleteConfig('${config.name}')">删除配置</button>
            `
          })
        })
      }
    ...
    applyConfigByName(name) {
      ...
      const config = configs.find(c => c.name === name)
      if (config) {
        this.showApplyConfirmDialog(config.name, config.data)
      }
    }

    // 删除配置
    deleteConfig(name) {
      this.showConfirmDialog('确认删除', `确定要删除配置"${name}"吗？`, () => {
        const configs = this.loadSavedConfigs()
        const newConfigs = configs.filter(c => c.name !== name)
        Storage.save('savedConfigs', newConfigs)
